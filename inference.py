    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if ghostPosition == jailPosition: #ghost is in jail
        	...
        	if noisyDistance == None:
	        	...
	        	pNoisyGivenTrue = busters.getObservationProbability(noisyDistance, trueDist)
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
        	newBeliefs[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        if newBeliefs.total() == 0:
        	self.initializeUniformly(gameState)
        else:
        	self.particles = [newBeliefs.sample() for _ in range(len(self.particles))]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                total *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
            newBeliefs[particle] += total
        if newBeliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [newBeliefs.sample() for _ in range(len(self.particles))]
